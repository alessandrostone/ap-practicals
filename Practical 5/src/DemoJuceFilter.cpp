...
//	highpassFilter.reset();
	...
	float lowCf = (params[LOWCF]*params[LOWCF]*9800) + 200;
	float bandCf = (params[BANDCF]*params[BANDCF]*9800) + 200;
	float bandQ = ((1-(0.9f*params[BANDQ]*params[BANDQ]))*10);
	float highCf = (params[HIGHCF]*params[HIGHCF]*9800) + 200;

//	lowpassFilter.createLowPass(currentSampleRate, lowCf);
	lowpassFilter.createBandPass(currentSampleRate, bandCf, bandQ);
	highpassFilter.createHighPass(currentSampleRate, highCf);
