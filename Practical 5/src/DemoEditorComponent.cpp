...
{
    AudioProcessor* currentFilter = getFilter();
	
	for (int i = 0; i < noParams; i++)
		if ( changedSlider == sliders[i] ) {
			currentFilter->setParameterNotifyingHost (i, (float) sliders[i]->getValue());
		}
}
...
void DemoEditorComponent::updateParametersFromFilter()
{
	...
	for(int i = 0; i < noParams; i++)
		tempParamVals[i] =  filter->getParameter (i);
...
{
	DemoJuceFilter* currentFilter = getFilter();
